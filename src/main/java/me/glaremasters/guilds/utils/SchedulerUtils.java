...
/**
 * Scheduler facade for Paper and Folia.
 *
 * Paper exposes the Folia scheduler APIs and internally maps them to the
 * appropriate scheduler on a normal Paper server, so the plugin does not
 * need to maintain separate scheduler implementations.
 */
public final class SchedulerUtils {
    ...
    public static ScheduledTask runGlobalRepeating(Plugin plugin, long initialDelayTicks, long periodTicks, Runnable task) {
        return plugin.getServer().getGlobalRegionScheduler().runAtFixedRate(plugin, ignored -> task.run(), initialDelayTicks, periodTicks);
    }
    ...
    public static ScheduledTask runAsyncLater(Plugin plugin, long delay, TimeUnit unit, Runnable task) {
        return plugin.getServer().getAsyncScheduler().runDelayed(plugin, ignored -> task.run(), delay, unit);
    }
}
